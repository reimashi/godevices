...
func Open(o driver.Opener, w, h int) (*OLED, error) {
	dev, err := spi.Open(o)
...
func (o *OLED) On() error {
	return o.dev.Tx([]byte{ssd1306DisplayOn}, nil)
}

// Off turns off the display if it is on.
func (o *OLED) Off() error {
	return o.dev.Tx([]byte{ssd1306DisplayOff}, nil)
}
...
func (o *OLED) Draw() error {
	if err := o.dev.Tx([]byte{
		0xa4,     // write mode
		0x40 | 0, // start line = 0
		0x21, 0, uint8(o.w),
		0x22, 0, 7,
	}, nil); err != nil { // the write mode
		return err
	}
	return o.dev.Tx(o.buf, nil)
}
